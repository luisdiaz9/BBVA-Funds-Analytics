...
button.on("click", function() {
    ...
    var new_table3 = new_table2.filter(info => info.shift===dateInputText.property("value"));
    if(Object.entries(new_table3).length===0 ){
      new_table3 = new_table2
    }
    //displayData(new_table3);    
    var dateInputText = d3.select("#debit");    
    var new_table4 = new_table3.filter(info => info.debit===parseFloat(dateInputText.property("value")));
    if(Object.entries(new_table4).length===0 ){
      new_table4 = new_table3
    }
    //displayData(new_table4);
    var dateInputText = d3.select("#credit");
    var new_table5 = new_table4.filter(info => info.credit===parseFloat(dateInputText.property("value")));
